...
ParserInfo compile(char* dir_name) {
    ...
    for (int i = 0; i < stdLibFilesCount; i++) {
        ...
        const char *base_name = strrchr(stdLibFiles[i], '/');
        base_name = base_name == NULL ? stdLibFiles[i] : base_name + 1;
        size_t symbol_len = strlen(base_name) - strlen(".jack");
        strncpy(symbol_name, base_name, symbol_len);
...
{
    if (argc != 2) {
        fprintf(stderr, "Usage: %s <jack-program-directory>\n", argv[0]);
        return EXIT_FAILURE;
    }
    ...
    if (p.er != none) {
        fprintf(stderr, "Compilation failed in %s at line %d near '%s' (error code %d)\n",
                p.tk.fl, p.tk.ln, p.tk.lx, p.er);
        return EXIT_FAILURE;
    }
