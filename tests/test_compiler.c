...
const char* testPrograms[NumberTestFiles] = {
	"Seven",
	"Fraction",
	"HelloWorld",
	"Square",
...
// Get a list of generated VM files in a directory.
int GetCodeFiles(const char *dir_name)
{
    DIR *dir = opendir(dir_name);
    if (dir == NULL) {
        fprintf(stderr, "Unable to open expected-output directory %s\n", dir_name);
        return 0;
    }
    ...
    while ((entry = readdir(dir)) != NULL && count < 128) {
        ...
        if (len > 3 && strcmp(entry->d_name + len - 3, ".vm") == 0) {
            code_files[count] = malloc(len + 1);
            if (code_files[count] == NULL) {
                closedir(dir);
                return count;
            }
            strcpy(code_files[count], entry->d_name);
            count++;
        }
    }
    closedir(dir);
...
int t_compiler ()
{
	...
	for (int j = 0 ; j < NumberTestFiles ; j++) // for each test file
	{
        ...
        snprintf(program_dir, sizeof(program_dir), "tests/fixtures/programs/%s", testPrograms[j]);
        ParserInfo p = compile(program_dir);
		StopCompiler ();
		if (p.er != none)
		{
		...
		else
		{
			...
			char full_dir_name[256];
			snprintf(full_dir_name, sizeof(full_dir_name), "tests/fixtures/expected/%s", testPrograms[j]);
			...
			for (int k = 0; k < num_vm_files ; k++)  // compare each vm file to the one generated by the compiler
			{
				...
				char vm_file_name[512];
				snprintf(vm_file_name, sizeof(vm_file_name), "%s/%s", full_dir_name, code_files[k]);
				...
				snprintf(vm_file_name, sizeof(vm_file_name), "%s/%s", program_dir, code_files[k]);
...
int main(void)
{
	...
	return tot == 20 ? EXIT_SUCCESS : EXIT_FAILURE;
}
#endif
